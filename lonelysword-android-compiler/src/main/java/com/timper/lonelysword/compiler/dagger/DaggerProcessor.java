...
public class DaggerProcessor extends AbstractProcessor {
  ...
  static final String MULTIMODULE_TYPE = "com.timper.lonelysword.dagger.MultiModule";
  ...
  static final         ClassName STRING      = ClassName.get("java.long", "String");
  ...
  public boolean process(Set<? extends TypeElement> set, RoundEnvironment env) {
    ...
    DaggerSet daggerSetApp = parseDaggerApplication(env);
    ...
    if (!Utils.isEmpty(moduleName)) {
      ...
      throw new RuntimeException("DaggerProcessor::Compiler >>> No module name, for more information, look at gradle log.");
    }
    //生成AppModule类
    ...
    //生成moduler支持dagger类
    if (daggerSetApp != null) {
      brewMultiModuleJava(daggerSetApp, bindingMap, moduleName);
    }
  ...
  private DaggerSet parseDaggerApplication(RoundEnvironment env) {
    //activity modules
    DaggerSet.Builder builder = null;
    ...
    for (Element element : env.getElementsAnnotatedWith(DaggerApplication.class)) {
      ...
      try {
        ...
        if (Utils.isSubtypeOfType(elementType, MULTIMODULE_TYPE)) {
          builder = parseMultiModuleDagger(element);
        }
      } catch (Exception e) {
  ...
  private DaggerSet.Builder parseMultiModuleDagger(Element element) {
    TypeElement enclosingElement = (TypeElement) element;
    ...
    return DaggerSet.newBuilder(enclosingElement, false);
  }
  ...
  private void brewActivityModuleJava(Map<TypeElement, DaggerSet> bindingMap, String moduleName) {
    try {
      JavaFile javaFile = JavaFile.builder(DAGGERPACKAGENAME, createActivityModuleType(bindingMap, moduleName, sdk, debuggable))
  ...
  private TypeSpec createMultiModuleType(DaggerSet binding, Map<TypeElement, DaggerSet> bindingMap, String moduleName, int sdk, boolean debuggable) {
    ...
    result.superclass(MOLTIMODULE);

    ParameterizedTypeName parameterizedTypeName = ParameterizedTypeName.get(MAP, STRING, MOLTIMODULE);
    ...
    StringBuilder statement = new StringBuilder(
      "DaggerMultiModule module = (DaggerMultiModule) Class.forName(" + binding.bindingClassName.enclosingClassName() + ")\n"
        + "                                                        .getConstructor()\n"
        + "                                                        .newInstance();\n");
