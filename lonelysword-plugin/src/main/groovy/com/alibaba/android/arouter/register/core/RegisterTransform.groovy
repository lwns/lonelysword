...
class RegisterTransform extends Transform {
    ...
    static ArrayList<ScanSetting> daggerRegisterList
    static File fileContainsInitClass;
    static File daggerFileContainsInitClass;
    ...
    void transform(Context context, Collection<TransformInput> inputs
        ...
        inputs.each { TransformInput input ->
            ...
            input.directoryInputs.each { DirectoryInput directoryInput ->
                ...
                directoryInput.file.eachFileRecurse { File file ->
                    ...
                    }else if(file.isFile() && ScanUtil.shouldDaggerProcessClass(path)){
                        ScanUtil.scanDaggerClass(file)
                    }
        ...
        if(daggerFileContainsInitClass){
            Logger.i('daggerFileContainsInitClass1: ')
            daggerRegisterList.each { ext ->
                //插入注册代码到
              if (ext.daggerClassList.isEmpty()){
                Logger.e("没发现需要注册的接口:" + ext.interfaceName)
              }else{
                ext.daggerClassList.each {
                  Logger.i(it)
                }
                RegisterCodeGenerator.insertDaggerInitCodeTo(ext)
              }
            }
        }
